...
- `--profile <selector>`: enable comma-separated diagnostic views. Startup selectors are `startup`, `startup:bootstrap`, `startup:imports`, and `startup:ink`. Interactive `composer` profiling shows a throttled summary for the current Left Arrow input burst below the prompt, retains the last completed burst until input resumes, and emits a machine-readable report after Ink exits; Audit and headless runs reject it.
