...
Ink renders completed turns once in `<Static>` history and keeps the current input, streaming output, tool status, and interactive decisions in a transient bottom area. The React tree is rendered once per process and unmounted only on exit. `logUserMessage` records submitted input immediately, avoiding a visual gap before model output begins. A proposed write is previewed transiently during confirmation; after it is applied, its diff is recorded as a tool-observation block and remains visible in conversation history.
