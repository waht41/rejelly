...
- `--profile <selector>`: enable comma-separated diagnostic views. Startup selectors are `startup`, `startup:bootstrap`, `startup:imports`, and `startup:ink`. Interactive `composer` profiling shows a throttled rolling input/commit summary below the prompt and emits a machine-readable report after Ink exits; Audit and headless runs reject it.
