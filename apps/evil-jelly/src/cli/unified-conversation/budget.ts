...
export function combineSessionBudget(
  ...
  return {
    ...
    cacheWriteTokens: (base?.cacheWriteTokens ?? 0) + (runAggregate.details?.cacheWriteTokens ?? 0),
    reasoningTokens: (base?.reasoningTokens ?? 0) + (runAggregate.details?.reasoningTokens ?? 0),
