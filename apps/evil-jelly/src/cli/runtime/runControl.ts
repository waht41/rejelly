 ...
 *
 * Distinct from shared task interruption (task-scoped, Esc / `/stop`, which
 * aborts only the active sub-agent and lets the run continue): this aborts the
 * whole `runWith` run. Aborting the run-level controller delivers the cancel
 * signal to the entire agent tree + teardown, so in-flight model/tool calls
 * cancel and the trace closes cleanly before the process exits.
 */
