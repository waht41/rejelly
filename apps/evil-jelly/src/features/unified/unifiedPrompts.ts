...
export function buildUnifiedSystemPrompt(options?: {
  ...
  builder.addList(
    [
      ...
      "If mcp_reference returns request_access or a relevant match with callable=false, call mcp_request once with the server id and a concise reason; do not ask the user to run /mcp manually. After access is granted, run mcp_reference again against the fresh dispatch. If access is denied, do not retry it in the same turn.",
      "For other unavailableServers states, report each structured status and follow its suggestedAction. Do not retry synonyms: pending, failed, and disabled are availability states, not search misses.",
