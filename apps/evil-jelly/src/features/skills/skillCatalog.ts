...
function toListItem(record: SkillRecord): SkillListItem {
  return Object.freeze({
    ...
    description: truncateSkillDisplayText(record.description, SKILL_LIMITS.listingDescriptionChars),
    ...(record.shortDescription
      ? {
          shortDescription: truncateSkillDisplayText(
            record.shortDescription,
            SKILL_LIMITS.listingDescriptionChars,
          ),
        }
      : {}),
...
export function createSkillCatalog(records: readonly SkillRecord[]): SkillCatalogSnapshot {
  ...
  return Object.freeze({
    ...
    list(cursor?: string): SkillListResult {
      ...
      for (
        let index = offset;
        index < listItems.length && items.length < SKILL_LIMITS.listPageEntries;
        index += 1
      ) {
