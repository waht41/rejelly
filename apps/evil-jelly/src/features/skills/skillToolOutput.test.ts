...
import type { SkillListPage, SkillRecord, SkillResourceEntry } from "./contracts";
...
describe("Skill tool output", () => {
  ...
  it("keeps the largest resource prefix that fits the hard Skill output limit", () => {
    const resources = Object.freeze(
      Array.from({ length: SKILL_LIMITS.resourcesPerSkill }, (_, index) =>
        Object.freeze({
    ...
    const output = renderSkillToolResult(
      record({ instruction: "x".repeat(SKILL_LIMITS.skillFileBytes), resources }),
    );

    expect(output.length).toBeLessThanOrEqual(SKILL_LIMITS.skillToolOutputChars);
  ...
  it("bounds list output and protects both listing and page boundaries", () => {
    ...
    expect(output.length).toBeLessThanOrEqual(SKILL_LIMITS.listToolOutputChars);
  ...
  it("renders canonical resource metadata and leaves content untouched", () => {
    ...
    expect(output.length).toBeLessThanOrEqual(SKILL_LIMITS.resourceToolOutputChars);
  ...
  it("returns a stable error when canonical resource metadata cannot fit", () => {
    const resource: SkillResourceEntry = Object.freeze({
      path: `references/${"&".repeat(SKILL_LIMITS.resourceToolOutputChars)}.md`,
      kind: "reference",
      sizeBytes: 1,
    });
    ...
    expect(output.length).toBeLessThan(SKILL_LIMITS.toolErrorMessageChars + 200);
  });
  ...
  it("renders stable bounded error envelopes", () => {
    ...
    expect(output.length).toBeLessThan(SKILL_LIMITS.toolErrorMessageChars + 100);
  });
});
