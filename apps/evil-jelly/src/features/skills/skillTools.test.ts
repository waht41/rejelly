...
import type { SkillRecord, SkillResourceReadResult, SkillRuntimeSnapshot } from "./contracts";
...
describe("Skill tools", () => {
  ...
  it("lists deterministic bounded pages and rejects malformed or foreign cursors", async () => {
    const records = Array.from({ length: SKILL_LIMITS.listPageEntries + 1 }, (_, index) =>
    ...
    expect(String(first).length).toBeLessThanOrEqual(SKILL_LIMITS.listToolOutputChars);
