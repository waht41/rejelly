...
import type { SkillLoadDiagnostic, SkillRecord, SkillResourceRepository } from "./contracts";
import { qualifiedSkillName, skillOrigin } from "./contracts";
...
async function collectSkillCandidates(source: SkillSource): Promise<SourceCandidatesResult> {
  ...
  if (candidates.length > SKILL_LIMITS.skillsPerSource) {
    diagnostics.push(
      skillDiagnostic(
        "skill.source.limit-exceeded",
        `The ${source.scope} Skill source exposes ${candidates.length} directories; only the first ${SKILL_LIMITS.skillsPerSource} were inspected.`,
        source.rootPath,
        origin,
      ),
    );
    candidates.length = SKILL_LIMITS.skillsPerSource;
  }
