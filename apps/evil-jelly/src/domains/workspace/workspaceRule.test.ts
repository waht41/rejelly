...
describe("readWorkspaceRuleMarkdown", () => {
  it("returns empty string when AGENTS.md does not exist", async () => {
    ...
    expect(await readWorkspaceRuleMarkdown(new WorkspaceFiles(cwd))).toBe("");
  });
  ...
  it("prefers AGENTS.override.md over AGENTS.md", async () => {
    ...
    expect(await readWorkspaceRuleMarkdown(new WorkspaceFiles(cwd))).toBe("Override rule");
  });
  ...
  it("reads AGENTS.md even when .gitignore hides it", async () => {
    ...
    expect(await readWorkspaceRuleMarkdown(new WorkspaceFiles(cwd))).toBe("Base rule");
  });
...
describe("buildWorkspaceRuleInstructionBlock", () => {
  it("wraps AGENTS.md in an XML-delimited block", async () => {
    ...
    const block = await buildWorkspaceRuleInstructionBlock(new WorkspaceFiles(cwd));
    expect(block).toBe(
  ...
  it("returns empty when AGENTS.md does not exist", async () => {
    const cwd = makeTempDir();
    expect(await buildWorkspaceRuleInstructionBlock(new WorkspaceFiles(cwd))).toBe("");
  });
});
