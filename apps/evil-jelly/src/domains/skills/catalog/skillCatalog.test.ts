...
import { qualifiedSkillName, type SkillRecord, skillOrigin } from "../definition/skillDefinition";
...
describe("SkillCatalog", () => {
  ...
  it("paginates with snapshot-bound opaque cursors and bounded DTO output", () => {
    const records = Array.from({ length: SKILL_CATALOG_LIMITS.listPageEntries + 5 }, (_, index) =>
    ...
    expect(first.page.returned).toBeLessThanOrEqual(SKILL_CATALOG_LIMITS.listPageEntries);
    expect(first.page.nextCursor).toBeTypeOf("string");
    expect(JSON.stringify(first.page).length).toBeLessThanOrEqual(
      SKILL_CATALOG_LIMITS.listPageOutputChars,
    );
