        ...
        if (depth >= SKILL_LOADER_LIMITS.resourceDirectoryDepth) {
          diagnostics.push(
            skillDiagnostic(
              ...
              `Resource directory depth exceeds ${SKILL_LOADER_LIMITS.resourceDirectoryDepth}; deeper resources are not listed.`,
              absolutePath,
              origin,
            ),
          );
...
export async function loadSkill(candidate: SkillLoadCandidate): Promise<SkillLoadResult> {
  ...
  const file = await readBoundedUtf8File(
    containedSkillFile.realPath,
    SKILL_LOADER_LIMITS.skillFileBytes,
  );
  if (!file.ok && file.reason === "too-large") {
    return failed(
      "skill.file.too-large",
      `SKILL.md exceeds ${SKILL_LOADER_LIMITS.skillFileBytes} bytes.`,
      skillFilePath,
    );
  }
  if (!file.ok) {
