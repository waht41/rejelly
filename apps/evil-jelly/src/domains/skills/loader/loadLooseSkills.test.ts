...
import { qualifiedSkillName, type SkillScope } from "../definition/skillDefinition";
...
describe("loose Skill source loading", () => {
  ...
  it("reports source truncation separately from invalid Skill files", async () => {
    ...
    await Promise.all(
      Array.from({ length: SKILL_LOADER_LIMITS.skillsPerSource + 1 }, (_, index) =>
    ...
    expect(result.diagnostics.filter((item) => item.code === "skill.file.invalid")).toHaveLength(
      SKILL_LOADER_LIMITS.skillsPerSource,
    );
  });
