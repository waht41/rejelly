...
import { z } from "zod";
import { validateSkillName } from "../definition/skillDefinition";
...
const metadataSchema = z
  .object({
    "short-description": z
      ...
      .trim()
      .min(1)
      .max(SKILL_LOADER_LIMITS.descriptionChars)
      .optional(),
  })
  .passthrough();
...
const frontmatterSchema = z
  .object({
    ...
    description: z.string().trim().min(1).max(SKILL_LOADER_LIMITS.descriptionChars),
...
function splitFrontmatter(
  ...
  while (lineStart <= text.length) {
    ...
    if (consumedBytes > SKILL_LOADER_LIMITS.frontmatterBytes) {
      return {
        ok: false,
        reason: `Frontmatter exceeds ${SKILL_LOADER_LIMITS.frontmatterBytes} bytes.`,
      };
    }
    if (line === "---") {
      return {
...
function inspectYamlNode(node: YamlNode | Pair | null, depth: number): string | undefined {
  ...
  if (depth > SKILL_LOADER_LIMITS.frontmatterDepth) {
    return `Frontmatter node depth exceeds ${SKILL_LOADER_LIMITS.frontmatterDepth}.`;
  }
  if (isAlias(node)) {
