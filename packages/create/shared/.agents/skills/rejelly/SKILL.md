...
<!-- AUTO-GENERATED from docs/ by packages/create/script/generate-agent-guidance.mjs. Do not edit by hand; run `pnpm --filter create-rejelly generate:guidance`. -->
...
- `createAgent(config)` (`references/api/core.md`): Define a reusable Agent; returns an async function.
- `promptAgent(schema)` (`references/api/core.md`): One typed model call per generation, with schema validation retries and the default tool-call loop.
- `promptChat(options?)` (`references/api/core.md`): Chat-style call returning `{ data, delta }` — `data` is the assistant text, `delta` is the new messages from this turn. Pass persisted history in via `message`; persist `delta` outside the Agent.
- `reborn(newProps?)` (`references/api/flow.md`): End the current generation and re-run the handler with Agent memory preserved.
- `runWith(fn, options?)` (`references/api/core.md`): Root runtime context — providers, model registry, snapshots, tracing, cancellation.

## Equip (context for the current generation)

- `equipSystem(text)` / `equipInstruction(text)` (`references/api/equip.md`): System and user-facing instructions.
- `equipTool(toolDef, options?)` (`references/api/equip.md`): Register a callable tool (Zod `parameters` + async `handler`). Per-tool middleware via `options.middleware`.
- `equipMemory(key, initialValue)` (`references/api/equip.md`): `[value, setter]` for JSON-serializable state; lives for one Agent invocation, survives `reborn()`.
- `equipResource(key, { create, destroy })` (`references/api/equip.md`): Non-serializable runtime objects with lifecycle cleanup.
- `equipScope(data)` (`references/api/equip.md`): Pass JSON-serializable context down to child Agents.
- `equipBudget(config)` (`references/api/budget.md`): Token and cost budgets.

## Expect (validation & dependencies)

- `expectValidator(fn)` (`references/api/expect.md`): Semantic validation beyond schema shape; return an error string to make the model retry with feedback.
- `expectScope(schema)` (`references/api/expect.md`): Read typed data from a parent's `equipScope()`.
- `expectResource<T>(key)` (`references/api/expect.md`): Read a resource exposed by an ancestor or injected via `runWith(fn, { providers })`.

## Effect & Middleware

- `onStream(consumer, options?)` (`references/api/effect.md`): Agent-level streaming events for UI and telemetry.
- `augmentModel` / `augmentTool` / `augmentAgent` (`references/api/core.md`): Reusable middleware around models, tools, and Agents.

# Going Deeper

Point users at these instead of inlining advanced material in first-contact examples:

- Custom prompt policies and orchestration beyond the default loop: `references/api/policy.md`
- Tool-call loop interception (`equipToolCallLoopMiddleware`): `references/api/core.md`
- Memoized computed values (`equipMemo`): `references/api/equip.md`
- Testing (`createMockModel`, test contexts): `references/api/testing.md`
- Snapshots and time travel (`dumpSnapshot` / `restoreSnapshot`): `references/api/time-travel.md`
- Tracing, loggers, `withCustomSpan`, `equipTraceAttr`: `references/api/debug.md`
- Model rate limiting (`withLimit`): `references/api/limit-model.md`
- Adapters — OpenAI, MCP (`equipMCP`), LangChain: `references/api/adapter/index.md`. These are separate packages; do not assume they are installed unless the user's project already includes them.
...
The `references/` directory is a versioned snapshot bundled with this release of `create-rejelly`. Read only the references relevant to the task. For the installed package's exact API surface, prefer the type definitions and CHANGELOG in `node_modules/@rejelly/core`. The latest complete documentation is available at https://docs.rejelly.dev/llm.txt.
