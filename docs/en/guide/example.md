...
Some example subdirectories provide a `README.md` (occasionally with `README.zh-CN.md`) containing run commands and detailed explanations. For examples without their own README, such as time-travel, refer to the unified start script and the corresponding `index.ts`. Install dependencies at the **`examples/`** root first.
