...
| Model Call | `model:call:start` / `:end` | Model adapter **physical request** open/close | start: `adapterId`, `provider`, `messageCount`, `usedTools`, `middlewares`; end: `rawText` / `reasoning` / `toolCalls`, `ttft`, `usage` (reasoning models may include `details.reasoningTokens`), `costs`, `finishReason` |
