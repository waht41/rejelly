...
**Actual position of this middleware**: Whenever a policy calls `executeTools(toolCalls, { runtime })` to execute a batch of `ToolCall[]`, the middleware chain runs **before** the underlying tools. Step 2 of the built-in `promptAgent` policy is one such caller. The middleware does not compose system/instruction content and does not replace per-tool `ToolMiddleware`.

> **⚠️ Scope: the `executeTools` batch path, NOT "global tool execution"**
>
> The built-in `promptAgent` tool round-trip loop triggers `ToolCallLoopMiddleware` through `executeTools`; a custom policy that calls the same primitive also triggers it. In contrast, directly executing one `ToolDefinition` via **`callTool(tool, args)`** uses the single-tool path and bypasses Loop middleware. Any other path that does not invoke `executeTools` also bypasses it. To cover manual single-tool calls, attach middleware at the **`equipTool(..., { middleware })` / `augmentTool`** layer. (`callTool` returns the handler's raw output and **throws** on failure; it does not fall back to a string.)
